...
{
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::Right) && Sprite.getPosition().x + size[0] * scale.x * 2 < bound[0]) //check right
	{
		if (sf::Keyboard::isKeyPressed(sf::Keyboard::Up) && Sprite.getPosition().y  > 0)//right+up
			Sprite.move(speed.x * 1 / sqrt(2), -speed.y * 1 / sqrt(2));
		else if (sf::Keyboard::isKeyPressed(sf::Keyboard::Down) && Sprite.getPosition().y + size[1] * scale.y * 2 < bound[1])//right+down
			Sprite.move(speed.x * 1 / sqrt(2), speed.y * 1 / sqrt(2));
		else //only right
	...
	else if (sf::Keyboard::isKeyPressed(sf::Keyboard::Left) && Sprite.getPosition().x  > 0)
	{
		if (sf::Keyboard::isKeyPressed(sf::Keyboard::Up) && Sprite.getPosition().y  > 0)//left+up
			Sprite.move(-speed.x * 1 / sqrt(2), -speed.y * 1 / sqrt(2));
		else if (sf::Keyboard::isKeyPressed(sf::Keyboard::Down) && Sprite.getPosition().y + size[1] * scale.y * 2 < bound[1])//left+down
			Sprite.move(-speed.x * 1 / sqrt(2), speed.y * 1 / sqrt(2));
		else //only left
			Sprite.move(-speed.x, 0);
	}
	else if (sf::Keyboard::isKeyPressed(sf::Keyboard::Down) && Sprite.getPosition().y + size[1] * scale.y * 2 < bound[1])
	{
